#!/usr/bin/env node
// Модуль для работы с Claude CLI
// Предоставляет утилиты для валидации подключения, выполнения команд и обработки результатов
// Claude CLI-related utility functions

// Динамическая загрузка модуля 'use' для работы с зависимостями
// If not, fetch it (when running standalone)
if (typeof globalThis.use === 'undefined') {
  globalThis.use = (await eval(await (await fetch('https://unpkg.com/use-m/use.js')).text())).use;
}
// Импорт зависимостей для работы с командами, файловой системой и путями
const { $ } = await use('command-stream');
const fs = (await use('fs')).promises;
const path = (await use('path')).default;

// Импорт утилит из других модулей
...
export const formatNumber = (num) => {
  ...
  // Преобразование в строку и разделение на целую и дробную части
  ...
  // Возврат с дробной частью, если она существует
  // Return with decimal part if it exists
  return decimalPart !== undefined
    ? `${formattedInteger}.${decimalPart}`
    : formattedInteger;
};
// Доступные конфигурации моделей Claude с сопоставлением коротких имен к полным ID
...
// Функция сопоставления алиасов моделей к полным идентификаторам
// Model mapping to translate aliases to full model IDs
export const mapModelToId = (model) => {
  return availableModels[model] || model;
};
// Функция валидации подключения к Claude CLI с логикой повторных попыток
// Проверяет доступность Claude CLI и корректность аутентификации
// Function to validate Claude CLI connection with retry logic
export const validateClaudeConnection = async (model = 'haiku-3') => {
  // Сопоставление алиаса модели к полному идентификатору
  // Map model alias to full ID
  const mappedModel = mapModelToId(model);
  // Конфигурация повторных попыток для ошибок перегрузки API
  // Retry configuration for API overload errors
  const maxRetries = 3;
  const baseDelay = timeouts.retryBaseDelay;
  let retryCount = 0;
  // Внутренняя функция попытки валидации с поддержкой повторов
  const attemptValidation = async () => {
    try {
      ...
      // Сначала выполняем быструю проверку версии CLI
      ...
      try {
        ...
        // Проверка версии не удалась, но продолжаем основную валидацию
      ...
      let result;
      try {
        // Основная валидация: используем pipe для передачи простого промпта
        // Primary validation: use printf piping with specified model
        result = await $`printf hi | claude --model ${mappedModel} -p`;
      } catch (pipeError) {
        // Если pipe не работает, используем альтернативный подход с timeout
        ...
        try {
          ...
          // Пробрасываем ошибку дальше, если это не таймаут
          // Re-throw if it's not a timeout error
          throw timeoutError;
        }
      }
    
      // Проверка распространенных паттернов ошибок в выводе команды
      // Check for common error patterns
      const stdout = result.stdout?.toString() || '';
      const stderr = result.stderr?.toString() || '';
      // Функция проверки JSON-ошибок в stdout или stderr
      // Check for JSON errors in stdout or stderr
      const checkForJsonError = (text) => {
        try {
          // Поиск JSON-паттернов с ошибками
          ...
          // Не валидный JSON, продолжаем другие проверки
      ...
      // Проверка на ошибку перегрузки API (500 Overloaded)
      ...
      // Обработка ошибок перегрузки с повторными попытками и экспоненциальной задержкой
      ...
      // Используем exitCode если code не определен (поведение Bun shell)
      ...
      // Проверка паттернов ошибок даже в успешных ответах (code=0)
      ...
      // Успех - Claude ответил (ответы LLM вероятностные, поэтому любой ответ считается хорошим)
      ...
      // Обработка исключений с повторными попытками для ошибок перегрузки
  ...
  }; // Конец функции attemptValidation / End of attemptValidation function
  // Запуск валидации с логикой повторных попыток
  // Start the validation with retry logic
  return await attemptValidation();
};
// Функция переключения runtime для Claude между Node.js и Bun
// Изменяет shebang в исполняемом файле Claude CLI
...
export const handleClaudeRuntimeSwitch = async (argv) => {
  if (argv['force-claude-bun-run']) {
    ...
    try {
      // Проверка наличия Bun runtime в системе
      try {
        await $`which bun`;
      ...
      // Поиск пути к исполняемому файлу Claude
      ...
      // Проверка прав на запись в файл Claude CLI
      try {
      ...
      // Чтение текущего shebang из первой строки файла
      ...
      // Чтение содержимого файла и замена shebang на bun
    ...
    // Выход после успешного переключения runtime
    // Exit after switching runtime
    process.exit(0);
  }

  // Обработка переключения обратно на Node.js runtime
  if (argv['force-claude-nodejs-run']) {
    await log('\n🔧 Restoring Claude runtime to Node.js...');
    try {
      // Проверка наличия Node.js runtime в системе
      try {
        await $`which node`;
      ...
      try {
        ...
        // Резервная копия отсутствует, обновляем shebang вручную
    ...
    // Выход после восстановления runtime
    // Exit after restoring runtime
    process.exit(0);
  }
};
/**
 * Выполнение Claude со всеми промптами и настройками
 * Главная точка входа, которая обрабатывает построение промптов и выполнение
...
export const executeClaude = async (params) => {
  // Деструктуризация параметров для выполнения Claude
  const {
    issueUrl,
    issueNumber,
    prNumber,
  ...
  // Импорт функций построения промптов из модуля claude.prompts.lib.mjs
  ...
  // Построение пользовательского промпта с контекстом задачи
  ...
  // Построение системного промпта с инструкциями и рекомендациями
  ...
  // Логирование деталей промптов в режиме verbose для отладки
  ...
  if (argv.verbose) {
    ...
    // В режиме dry-run выводим фактическое содержимое промптов для отладки
  ...
  // Экранирование промптов для использования в shell-командах
  // Escape prompts for shell usage
  const escapedPrompt = prompt.replace(/"/g, '\\"').replace(/\$/g, '\\$');
  const escapedSystemPrompt = systemPrompt.replace(/"/g, '\\"').replace(/\$/g, '\\$');
  // Выполнение команды Claude с подготовленными промптами
  ...
  return await executeClaudeCommand({
    ...
    claudePath,
    $,
    // Параметры для интерактивного режима (комментарии в PR в реальном времени)
...
/**
 * Вычисление общего использования токенов из JSONL-файла сессии
...
/**
 * Получение информации о модели из API с ценами
...
export const fetchModelInfo = async (modelId) => {
  try {
    ...
    return new Promise((resolve, reject) => {
      // Запрос к API models.dev для получения информации о модели
    ...
    // Если не удается получить информацию о модели, возвращаем null и продолжаем без неё
    // If we can't fetch model info, return null and continue without it
    return null;
  }
};
/**
 * Вычисление стоимости использования модели в USD с детальной разбивкой
...
export const calculateModelCost = (usage, modelInfo, includeBreakdown = false) => {
  ...
  // Структура для детальной разбивки стоимости по типам токенов
  const breakdown = {
  ...
  // Расчет стоимости входных токенов (за миллион токенов)
  ...
  // Расчет стоимости создания кеша (cache write)
  ...
  // Расчет стоимости чтения из кеша (cache read, дешевле входных токенов)
  ...
  // Расчет стоимости выходных токенов (обычно дороже входных)
  // Output tokens cost
  ...
  // Общая стоимость как сумма всех компонентов
