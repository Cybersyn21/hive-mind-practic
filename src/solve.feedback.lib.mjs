/**
 * Модуль обнаружения обратной связи для solve.mjs
 * Обрабатывает подсчет комментариев и обнаружение feedback для режима продолжения работы
 * Анализирует новые комментарии в PR и issue, определяет требуется ли повторный запуск AI
 */

// Импортируем интеграцию Sentry для отчетов об ошибках
import { reportError } from './sentry.lib.mjs';

// Обнаруживает и подсчитывает обратную связь (feedback) в PR и issue
// Возвращает feedbackLines для передачи AI инструменту и количество новых комментариев
