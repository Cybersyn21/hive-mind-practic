/**
 * Модуль создания и переключения веток для solve.mjs
 * Обрабатывает создание новых веток или переключение на существующие ветки PR
 * Поддерживает паттерны имен веток issue-{number}-{hex} в legacy (8 char) и новом (12 char) форматах
 */

/**
 * Регулярные выражения для валидации имен веток
 * Поддерживает как legacy (8-char) так и новый (12-char) форматы
 * Формат: issue-{номер_issue}-{случайный_hex_id}
 */
