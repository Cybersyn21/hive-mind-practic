...
/**
 * Модуль локальных CI проверок
 * Обнаруживает и запускает локальные CI проверки перед пушем чтобы избежать сбоев CI
 * Поддерживает Python (ruff, mypy, pytest), JavaScript (eslint, prettier), Rust (cargo)
 */
...
/**
 * Обнаруживает какие CI инструменты настроены в проекте
 * Проверяет наличие конфигурационных файлов для различных линтеров и тестовых фреймворков
 * @param {string} workDir - Рабочая директория
 * @returns {Promise<Object>} Обнаруженные CI инструменты и их конфигурации
 */
