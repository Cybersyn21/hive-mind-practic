...
// Ленивая загрузка конфигурации только при необходимости, чтобы избежать загрузки use-m при инициализации модуля
// Это предотвращает сетевые запросы, которые могут зависнуть во время --help или --version

// Check if Sentry should be disabled
// Проверка, должен ли Sentry быть отключен
const shouldDisableSentry = () => {
  // Check for --no-sentry flag
  // Проверка флага --no-sentry
  ...
  // Проверка CI окружения (отключаем в CI по умолчанию)
  ...
  // Отключаем Sentry для быстрых команд, которым не нужно отслеживание ошибок
  // Это предотвращает блокировку выхода из процесса интеграцией профилирования Sentry
  ...
  // Отключаем Sentry для режима dry-run, чтобы избежать ненужных сетевых вызовов
  // Это предотвращает загрузку use-m из CDN в тестовых сценариях
...
// Ленивый импорт Sentry только при необходимости
// Это предотвращает удержание event loop пакетами Sentry когда они не нужны
let Sentry = null;
let nodeProfilingIntegration = null;

// Initialize Sentry if not disabled
// Инициализация Sentry если не отключен
if (!shouldDisableSentry()) {
  try {
    ...
    // Динамический импорт конфигурации только когда Sentry фактически инициализируется
    // Это избегает загрузки use-m до обработки аргументов командной строки
    const { sentry, version } = await import('./config.lib.mjs');

    // Dynamically import Sentry packages only when needed
    // Динамический импорт пакетов Sentry только когда необходимо
    ...
    // Инициализация Sentry с конфигурацией
    Sentry.init({
      dsn: sentry.dsn,
      integrations: [
      ...
      // Отправка структурированных логов в Sentry
      enableLogs: true,
      ...
      // Установка частоты выборки для профилирования
      ...
      // Lifecycle трассировки автоматически включает профилирование во время активных трасс
      profileLifecycle: 'trace',

      // Setting this option to true will send default PII data to Sentry
      // Установка в true отправит персональные данные в Sentry
      sendDefaultPii: false, // Disabled for privacy / Отключено для конфиденциальности

      // Set debug mode based on environment
      // Установка режима отладки на основе окружения
      debug: process.env.DEBUG === 'true' || process.env.NODE_ENV === 'development',

      // Before send hook to filter out sensitive data
      // Хук перед отправкой для фильтрации чувствительных данных
      beforeSend(event) {
