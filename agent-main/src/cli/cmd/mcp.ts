...
// Команда управления MCP серверами (Model Context Protocol)
// Позволяет добавлять и просматривать список MCP серверов
...
// Загрузить глобальную конфигурацию opencode
// Возвращает существующую конфигурацию или создает дефолтную структуру
async function loadGlobalConfig(): Promise<Config.Info> {
  ...
  try {
    ...
    // Если файл не существует, возвращаем базовую конфигурацию
    return {
...
// Сохранить глобальную конфигурацию в файл
...
// Команда добавления нового MCP сервера
// Поддерживает как интерактивный режим, так и CLI аргументы
export const McpAddCommand = cmd({
  ...
  async handler(args) {
    // Если имя и команда/url переданы как аргументы CLI, используем неинтерактивный режим
    if (args.name && ((args.command && args.command.length > 0) || args.url)) {
      // Неинтерактивный режим: все параметры предоставлены через CLI
      const config = await loadGlobalConfig()
      config.mcp = config.mcp || {}

      if (args.url) {
        // Добавляем удаленный MCP сервер
        ...
        // Добавляем локальный MCP сервер
        config.mcp[args.name] = {
          type: "local",
    ...
    // Интерактивный режим: запрашиваем данные у пользователя через промпты
    UI.empty()
    ...
    // Выбираем тип сервера (локальный или удаленный)
    ...
    if (type === "local") {
      // Конфигурируем локальный MCP сервер
      ...
      // Разбиваем команду на массив аргументов
    ...
    if (type === "remote") {
      // Конфигурируем удаленный MCP сервер
      ...
      // Проверяем соединение с удаленным сервером
      try {
...
// Команда отображения списка сконфигурированных MCP серверов
// Выводит информацию о каждом сервере с его параметрами и статусом
export const McpListCommand = cmd({
  ...
  async handler() {
    ...
    // Перебираем и выводим информацию о каждом сервере
    for (const [name, server] of Object.entries(mcpServers)) {
      // Отображаем статус включено/выключено цветом
      ...
      // Выводим детали конфигурации в зависимости от типа сервера
