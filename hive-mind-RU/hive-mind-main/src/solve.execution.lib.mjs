...
// Модуль основной логики выполнения для команды solve
// Извлечен из solve.mjs для поддержания размера файлов под 1500 строк
// Содержит функции настройки временных директорий, выполнения AI инструментов и обработки результатов

// Используем use-m для динамического импорта модулей для кросс-рантайм совместимости
// Проверяем определен ли use глобально (когда импортирован из solve.mjs)
// Если нет, загружаем его (когда запускается standalone)
if (typeof globalThis.use === 'undefined') {
  globalThis.use = (await eval(await (await fetch('https://unpkg.com/use-m/use.js')).text())).use;
}
const use = globalThis.use;

// Используем command-stream для консистентного поведения $ в разных рантаймах
const { $ } = await use('command-stream');

const os = (await use('os')).default;
const path = (await use('path')).default;
const fs = (await use('fs')).promises;
// crypto модуль не используется, удален для исправления linting

// Импортируем функции проверки памяти (RAM, swap, disk)
const memoryCheck = await import('./memory-check.mjs');

// Импортируем функции общей библиотеки
...
// Импортируем функции работы с GitHub
const githubLib = await import('./github.lib.mjs');
// Импортируем интеграцию Sentry
...
// Создает или находит временную директорию для клонирования репозитория
// Обрабатывает режим resume для продолжения с предыдущей сессии
