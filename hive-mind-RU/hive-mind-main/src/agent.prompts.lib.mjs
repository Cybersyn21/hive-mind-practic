/**
 * Модуль промптов для Agent
 * Управляет построением промптов для команд Agent
 */

/**
 * Построение пользовательского промпта для Agent
 * Формирует краткий и четкий промпт с контекстом задачи, PR и обратной связью
 * @param {Object} params - Параметры для построения пользовательского промпта
 * @returns {string} Отформатированный пользовательский промпт
 */
export const buildUserPrompt = (params) => {
  ...
  if (argv && argv.fork && forkedRepo) {
    ...
    // Проверка наличия GitHub Actions на форке
  ...
  // Добавляем обратную связь в режиме продолжения
  if (isContinueMode && feedbackLines && feedbackLines.length > 0) {
    // Добавляем каждую строку обратной связи напрямую
    feedbackLines.forEach(line => promptLines.push(line));
    promptLines.push('');
  }

  // Добавляем инструкцию для размышления в зависимости от уровня --think
  if (argv && argv.think) {
    const thinkMessages = {
      low: 'Think.',          // Немного подумай
      medium: 'Think hard.',  // Хорошо подумай
      high: 'Think harder.',  // Очень хорошо подумай
      max: 'Ultrathink.'      // Максимально глубоко размысли
    };
...
/**
 * Построение системного промпта для Agent - адаптированный под возможности Agent
 * Содержит подробные инструкции по решению задач, исследованию, тестированию и работе с GitHub
 * @param {Object} params - Параметры для построения промпта
 * @returns {string} Отформатированный системный промпт
 */
export const buildSystemPrompt = (params) => {
  const { owner, repo, issueNumber, prNumber, branchName, argv } = params;

  // Формируем инструкцию для размышления на основе уровня --think
  let thinkLine = '';
  if (argv && argv.think) {
    const thinkMessages = {
      low: 'You always think on every step.',       // Всегда думай на каждом шаге
      medium: 'You always think hard on every step.', // Всегда хорошо думай на каждом шаге
      high: 'You always think harder on every step.', // Всегда очень хорошо думай на каждом шаге
      max: 'You always ultrathink on every step.'     // Всегда максимально глубоко размышляй на каждом шаге
    };
