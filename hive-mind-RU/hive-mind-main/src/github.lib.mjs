#!/usr/bin/env node
// Библиотека утилит для работы с GitHub
// Включает функции для работы с API GitHub, управления репозиториями, PR, issues и правами доступа
...
// Используем command-stream для единообразного поведения команды $
const { $ } = await use('command-stream');
// Импортируем функции логирования и маскирования токенов из общей библиотеки
import { log, maskToken, cleanErrorMessage } from './lib.mjs';
import { reportError } from './sentry.lib.mjs';
import { githubLimits, timeouts } from './config.lib.mjs';
// Импортируем пакетные операции из отдельного модуля
import {
...
/**
 * Построение строки оценки стоимости для комментариев в логах
 * Форматирует информацию о стоимости выполнения AI операций в markdown-формате
 * @param {number|null} totalCostUSD - Публичная оценка стоимости
 * @param {number|null} anthropicTotalCostUSD - Стоимость, рассчитанная Anthropic (специфично для Claude)
 * @param {Object|null} pricingInfo - Информация о ценах от инструмента agent
 * @returns {string} Отформатированная строка с информацией о стоимости для markdown
 */
