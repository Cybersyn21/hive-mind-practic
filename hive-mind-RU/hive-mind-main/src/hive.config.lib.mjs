// Модуль конфигурации CLI для команды hive
// Извлечен из hive.mjs чтобы избежать загрузки тяжелых зависимостей (instrument.mjs и т.д.)
// когда нужна только конфигурация yargs (например, в telegram-bot.mjs)
// Этот модуль не имеет тяжелых зависимостей для быстрой загрузки при --help

// Создает конфигурацию yargs для команды hive
// Определяет все доступные опции для мониторинга и обработки GitHub issues
export const createYargsConfig = (yargsInstance) => {
  return yargsInstance
    ...
    .fail((msg, err) => {
      // Кастомный обработчик ошибок для подавления автоматического вывода ошибок yargs в stderr
      // Ошибки обрабатываются в try-catch блоке вызывающего кода
      // Если есть существующий объект ошибки, пробрасываем его как есть для сохранения полного стека
      if (err) {
        throw err;
      }
      // Для сообщений валидации пробрасываем их как есть без обертки
      // Это сохраняет оригинальную ошибку и её стек вызовов
      const error = new Error(msg);
      // Сохраняем оригинальную ошибку как причину если yargs её предоставил
      if (err) {
        error.cause = err;
      }
      throw error;
