#!/usr/bin/env node
/**
 * ГЛАВНЫЙ ИСПОЛНЯЕМЫЙ ФАЙЛ КОМАНДЫ SOLVE
 *
 * Решает GitHub issues и создает Pull Request'ы используя AI инструменты (Claude, OpenCode, Codex, Agent).
 * Этот модуль координирует весь процесс работы с задачей: от парсинга URL до создания и верификации PR.
 *
 * ОСНОВНОЙ ПРОЦЕСС:
 * 1. Парсинг и валидация аргументов командной строки (GitHub URL, опции)
 * 2. Проверка системных ресурсов (диск, память) и подключения к инструментам
 * 3. Настройка репозитория: клонирование, создание форка (если нужно), настройка remotes
 * 4. Создание/переключение на рабочую ветку (issue-{number}-{random-id})
 * 5. Автоматическое создание draft PR с CLAUDE.md файлом (опционально)
 * 6. Сбор контекста: обнаружение feedback, временные метки, проверка незакоммиченных изменений
 * 7. Запуск AI инструмента с подготовленным контекстом
 * 8. Обработка результатов: очистка файлов, верификация PR, прикрепление логов
 * 9. Режим наблюдения (--watch): непрерывный мониторинг feedback и автоматические перезапуски
 *
 * ПОДДЕРЖИВАЕМЫЕ РЕЖИМЫ:
 * - Новая задача: создает новую ветку и PR для issue
 * - Режим продолжения (--auto-continue): продолжает работу с существующим PR
 * - Режим возобновления (--resume): продолжает прерванную сессию после достижения лимита
 * - Режим наблюдения (--watch): мониторит PR на предмет feedback и автоматически перезапускается
 * - Dry-run режим (--dry-run): только подготовка без выполнения
 *
 * АРХИТЕКТУРА:
 * Модульная структура с разделением ответственности:
 * - solve.config.lib.mjs: конфигурация CLI
 * - solve.validation.lib.mjs: валидация входных данных
 * - solve.repository.lib.mjs: операции с репозиториями
 * - solve.branch.lib.mjs: управление ветками
 * - solve.preparation.lib.mjs: подготовка к выполнению
 * - solve.execution.lib.mjs: логика выполнения
 * - solve.feedback.lib.mjs: обнаружение обратной связи
 * - solve.results.lib.mjs: обработка результатов
 * - solve.watch.lib.mjs: режим наблюдения
 * - solve.error-handlers.lib.mjs: централизованная обработка ошибок
 */

// Импортируем Sentry для мониторинга ошибок (должен быть первым для перехвата всех ошибок)
import './instrument.mjs';
// Пути быстрого выхода - обрабатываем до загрузки всех модулей для ускорения тестов
