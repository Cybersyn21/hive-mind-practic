...
/**
 * Модуль режима наблюдения (watch mode) для solve.mjs
 * Непрерывно мониторит обратную связь и перезапускается когда обнаруживаются изменения
 * Поддерживает два режима: постоянное наблюдение (--watch) и временное для незакоммиченных изменений
 */
