...
// Интерфейс для статистики по сессиям
// Содержит агрегированные данные о токенах, стоимости и использовании инструментов
interface SessionStats {
  totalSessions: number // Общее количество сессий
  totalMessages: number // Общее количество сообщений
  totalCost: number // Общая стоимость в USD
  totalTokens: {
    input: number // Входные токены
    output: number // Выходные токены
    reasoning: number // Токены reasoning (для моделей с CoT)
    cache: {
      read: number // Прочитанные из кеша токены
      write: number // Записанные в кеш токены
    }
  }
  toolUsage: Record<string, number> // Количество использований каждого инструмента
  dateRange: {
    earliest: number // Самая ранняя дата (Unix timestamp)
    latest: number // Самая поздняя дата (Unix timestamp)
  }
  days: number // Количество дней в диапазоне
  costPerDay: number // Средняя стоимость за день
}

// Команда отображения статистики использования токенов и стоимости
...
// Получить все сессии из всех проектов
// Проходит по всем проектам в storage и собирает их сессии
...
// Агрегировать статистику по сессиям с фильтрацией
// Собирает данные о токенах, стоимости и использовании инструментов
async function aggregateSessionStats(days?: number, projectFilter?: string): Promise<SessionStats> {
  ...
  // Фильтруем сессии по дате если указано
  ...
  if (projectFilter !== undefined) {
    if (projectFilter === "") {
      // Пустая строка означает текущий проект
      const currentProject = await getCurrentProject()
      filteredSessions = filteredSessions.filter((session) => session.projectID === currentProject.id)
    } else {
      // Фильтруем по конкретному ID проекта
      filteredSessions = filteredSessions.filter((session) => session.projectID === projectFilter)
    }
  }

  // Инициализируем структуру статистики
  ...
  // Обрабатываем сессии батчами для оптимизации памяти
  const BATCH_SIZE = 20
