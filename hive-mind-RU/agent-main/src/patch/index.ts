...
// Пространство имен для работы с патчами файлов
// Реализует парсинг и применение патчей в специальном формате
// Поддерживает добавление, удаление и обновление файлов
export namespace Patch {
  const log = Log.create({ service: "patch" })

  // Zod схема для параметров патча
  ...
  // Основные типы, соответствующие реализации на Rust
  // Аргументы для применения патча
  export interface ApplyPatchArgs {
    patch: string // Текст патча
    hunks: Hunk[] // Фрагменты изменений
    workdir?: string // Рабочая директория
  }

  // Фрагмент изменения (hunk) - может быть добавлением, удалением или обновлением файла
  export type Hunk =
    | { type: "add"; path: string; contents: string } // Добавление нового файла
    | { type: "delete"; path: string } // Удаление файла
    | { type: "update"; path: string; move_path?: string; chunks: UpdateFileChunk[] } // Обновление файла

  // Фрагмент обновления файла - описывает изменения в определенной части файла
  export interface UpdateFileChunk {
    old_lines: string[] // Старые строки (что заменяем)
    new_lines: string[] // Новые строки (на что заменяем)
    change_context?: string // Контекст для поиска места изменения
    is_end_of_file?: boolean // Флаг конца файла
  }

  // Действие применения патча с картой изменений
  export interface ApplyPatchAction {
    changes: Map<string, ApplyPatchFileChange> // Карта изменений по путям файлов
    patch: string // Текст патча
  ...
  export type ApplyPatchFileChange =
    | { type: "add"; content: string } // Добавление файла
    | { type: "delete"; content: string } // Удаление файла
    | { type: "update"; unified_diff: string; move_path?: string; new_content: string } // Обновление файла

  // Пути затронутых файлов после применения патча
  export interface AffectedPaths {
    added: string[] // Добавленные файлы
    modified: string[] // Измененные файлы
    deleted: string[] // Удаленные файлы
  }

  // Типы ошибок при применении патча
  export enum ApplyPatchError {
    ParseError = "ParseError", // Ошибка парсинга
    IoError = "IoError", // Ошибка ввода-вывода
    ComputeReplacements = "ComputeReplacements", // Ошибка вычисления замен
    ImplicitInvocation = "ImplicitInvocation", // Неявный вызов (патч без команды)
  }

  // Результат попытки распарсить команду apply_patch
  export enum MaybeApplyPatch {
    Body = "Body", // Успешно распарсено
    ShellParseError = "ShellParseError", // Ошибка парсинга shell команды
    PatchParseError = "PatchParseError", // Ошибка парсинга патча
    NotApplyPatch = "NotApplyPatch", // Не команда apply_patch
  }

  // Результат проверенного парсинга apply_patch
  export enum MaybeApplyPatchVerified {
    Body = "Body", // Успешно проверено
    ShellParseError = "ShellParseError", // Ошибка парсинга shell
    CorrectnessError = "CorrectnessError", // Ошибка корректности
    NotApplyPatch = "NotApplyPatch", // Не команда apply_patch
  }

  // Реализация парсера патчей
  // Парсит заголовок патча (*** Add File:, *** Delete File:, *** Update File:)
  function parsePatchHeader(
    ...
    // Парсим директиву добавления файла
    if (line.startsWith("*** Add File:")) {
      const filePath = line.split(":", 2)[1]?.trim()
      return filePath ? { filePath, nextIdx: startIdx + 1 } : null
    }

    // Парсим директиву удаления файла
    if (line.startsWith("*** Delete File:")) {
      const filePath = line.split(":", 2)[1]?.trim()
      return filePath ? { filePath, nextIdx: startIdx + 1 } : null
    }

    // Парсим директиву обновления файла (может включать перемещение)
    if (line.startsWith("*** Update File:")) {
      const filePath = line.split(":", 2)[1]?.trim()
      let movePath: string | undefined
      let nextIdx = startIdx + 1

      // Проверяем наличие директивы перемещения
  ...
  // Парсит фрагменты обновления файла (chunks)
  // Каждый chunk содержит контекст (@@ ...) и изменения (-, +, пробел)
