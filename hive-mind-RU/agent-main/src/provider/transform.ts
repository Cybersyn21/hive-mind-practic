...
// Пространство имен для трансформации данных для разных провайдеров AI
// Обрабатывает специфичные для провайдеров требования к сообщениям, параметрам и кэшированию
export namespace ProviderTransform {
  // Нормализует сообщения для конкретного провайдера
  // Обрабатывает специфичные требования к ID инструментов и порядку сообщений
  function normalizeMessages(msgs: ModelMessage[], providerID: string, modelID: string): ModelMessage[] {
    // Обработка для моделей Claude
    // Claude требует, чтобы toolCallId содержал только буквы, цифры, _ и -
    ...
    // Обработка для моделей Mistral
    // Mistral имеет строгие требования к формату ID и порядку сообщений
